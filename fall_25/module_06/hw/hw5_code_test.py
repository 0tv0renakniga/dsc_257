...
def model_ll(model, text):
    ...
    for context, target in ngrams:
        # If an n-gram hasn't been seen in training, get_prob will KeyError.
        # We follow the provided API and surface this as a math domain issue (no smoothing).
        try:
            prob = model.get_prob(context, target)
        except KeyError:
            # Unseen n-gram implies zero probability under MLE; log-likelihood is -inf.
            return float("-inf")
        # Natural logarithm
        ll += math.log(prob)
...
## SOLUTION:
# Create unigram (n=1) and reuse the bigram (n=2) from Part (a)
unigram_model = create_and_fit_model(n=1, sentences=sentences)

# Compute log-likelihoods using natural log
unigram_ll = model_ll(unigram_model, text)
bigram_ll = model_ll(bigram_model, text)

print("--- Part (b): Log-likelihoods ---")
print(f"Unigram log-likelihood: {unigram_ll}")
print(f"Bigram log-likelihood: {bigram_ll}")
...
# Generate and report exactly 20 tokens from both models using the same prompt
print("\n--- Part (c): 20-word continuations from prompt ---")
print(f"Prompt: {prompt}")

bigram_20 = generate_text(bigram_model, 20, prompt=prompt)
trigram_20 = generate_text(trigram_model, 20, prompt=prompt)

print("Bigram Model (n=2) - 20 words:")
print(bigram_20)
print("Trigram Model (n=3) - 20 words:")
print(trigram_20)
