...
\end{lstlisting}
\subsubsection*{\normalfont}{$\therefore$ There are 66 unique characters in the vocabulary. The integer code for 'A' is 12}
...
\subsubsection*{Python Code}
\begin{lstlisting}[language=Python]
print(f"Number of characters in shakespeare_plays.txt: {len(data)}")
\end{lstlisting}
\subsubsection*{\normalfont}{$\therefore$ There are 3801088 characters in \texttt{shakespeare\_plays.txt}}
...
The size 66 is used for both the input and output layers because it corresponds to the total number of unique characters in the vocabulary as found in \textit{part (a)}.
