...
\begin{align*}
    % Step 1: Define the condition for a single heavy hitter
    \text{By definition, for each of the } k \text{ heavy hitters:} \\
    f_i &\geq \epsilon m \\
    % Step 2: State the constraint on the sum of frequencies
    \text{The sum of frequencies of these distinct items cannot exceed the total stream length } m\text{:} \\
    \sum_{i=1}^{k} f_i &\leq m \\
    % Step 3: Combine the two constraints
    \text{Substituting the minimum frequency for each heavy hitter into the sum:} \\
    \sum_{i=1}^{k} (\epsilon m) &\leq \sum_{i=1}^{k} f_i \leq m \\
    % Step 4: Simplify the inequality
    k \cdot \epsilon \cdot m &\leq m \\
    % Step 5: Solve for k
    \text{Since } m > 0 \text{, we can divide by } m \text{:} \\
    k \cdot \epsilon &\leq 1 \\
    k &\leq \frac{1}{\epsilon} \\
    % Step 6: Substitute the given value for epsilon
    \text{Substituting } \epsilon = 0.05\text{:} \\
    k &\leq \frac{1}{0.05} \\
    k &\leq 20
\end{align*}

\subsubsection*{\normalfont}{$\therefore$ $k \leq 20$ demonstrates that for $\epsilon=0.05$, there can be at most \textbf{20} $\epsilon$-heavy hitters in the stream.}
